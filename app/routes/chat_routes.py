...
# ---------------------------------------------------------------------------
# Company KPI dashboard (chat sidebar)
#
# Serves the standard KPIs extracted from each quarterly report as plain JSON.
# No LLM is involved at read time — values come verbatim from the extraction
# JSON (which itself carries the source quote from the document), so the
# dashboard can never invent a number.
# ---------------------------------------------------------------------------
...
def company_kpis(
    ...
    from ..auth import user_can_access_company
    ...
    return JSONResponse({
        ...
        "periods": periods,   # oldest → newest; last entry = latest quarter
    })
